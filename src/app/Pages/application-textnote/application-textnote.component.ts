...
@Component({
  selector: 'app-application-textnote',
  templateUrl: './application-textnote.component.html',
  styleUrls: ['./application-textnote.component.scss']
})
export class ApplicationTextnoteComponent implements OnInit {
  AppForm: FormGroup;
  eventId: any;
  id: any;
  slug: any;
  ...
  submit() {
    if (this.AppForm.value.content) {
      ...
      // data.append('title', this.AppForm.value.title);
      if(this.eventId){
        data.append('id', this.eventId)
      }
      data.append('content', this.AppForm.value.content);
      data.append('slug', 'notes');
      data.append('title', 'Text/Note');
